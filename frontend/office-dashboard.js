...
.meos-hallway-mini-action[data-kind="take-it"] { border-color: rgba(126, 235, 173, 0.5); color: #bff5d3; }
      ...
      .meos-maddy-response-overlay{position:fixed;inset:0;z-index:100060;display:none;align-items:center;justify-content:center;padding:clamp(16px,3vw,42px);background:rgba(0,7,16,.78);backdrop-filter:blur(8px)}
      .meos-maddy-response-overlay[data-open="true"]{display:flex}
      .meos-maddy-response-modal{width:min(920px,96vw);max-height:min(820px,90vh);display:flex;flex-direction:column;overflow:hidden;border:1px solid rgba(111,229,255,.5);border-radius:18px;background:linear-gradient(155deg,rgba(3,17,31,.99),rgba(5,31,47,.985));box-shadow:0 32px 100px rgba(0,0,0,.62),0 0 34px rgba(78,210,255,.12);color:#eaf9ff}
      .meos-maddy-response-head{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;padding:20px 22px 15px;border-bottom:1px solid rgba(119,216,241,.18)}
      .meos-maddy-response-kicker{font-size:.66rem;font-weight:900;letter-spacing:.18em;text-transform:uppercase;color:#7ee8f7}
      .meos-maddy-response-title{margin:6px 0 0;font-size:clamp(1.05rem,2vw,1.35rem);line-height:1.3;color:#fff}
      .meos-maddy-response-close{flex:0 0 auto;width:38px;height:38px;border:1px solid rgba(125,220,246,.38);border-radius:11px;background:rgba(10,38,58,.72);color:#dff8ff;font-size:1.25rem;cursor:pointer}.meos-maddy-response-close:hover{border-color:rgba(126,235,255,.9)}
      .meos-maddy-response-body{overflow:auto;padding:20px 22px 22px}
      .meos-maddy-response-answer{margin:0;font-size:clamp(.92rem,1.45vw,1.06rem);line-height:1.72;color:#e7f5fb;white-space:pre-wrap}
      .meos-maddy-response-status{margin:0 0 15px;padding:12px 14px;border:1px solid rgba(250,198,91,.28);border-radius:11px;background:rgba(88,59,11,.2);font-size:.82rem;line-height:1.55;color:#ffe9b0}
      .meos-maddy-response-assignment{margin-top:16px;padding:13px 14px;border:1px solid rgba(129,170,192,.18);border-radius:11px;background:rgba(0,9,18,.44);color:#9fb8c7;font-size:.76rem;line-height:1.55;white-space:pre-wrap}
      .meos-maddy-response-assignment strong{display:block;margin-bottom:6px;color:#d4e7f1;font-size:.62rem;letter-spacing:.12em;text-transform:uppercase}
      .meos-maddy-response-meta{display:flex;gap:8px;flex-wrap:wrap;margin:0 0 14px}.meos-maddy-response-chip{display:inline-flex;align-items:center;padding:5px 8px;border:1px solid rgba(95,210,239,.2);border-radius:999px;background:rgba(14,47,68,.58);font-size:.65rem;color:#bfe8f3}
      .meos-maddy-response-sources{margin-top:18px;padding-top:14px;border-top:1px solid rgba(117,206,231,.14)}.meos-maddy-response-sources-label{display:block;margin-bottom:8px;font-size:.62rem;font-weight:800;letter-spacing:.13em;text-transform:uppercase;color:#799dad}.meos-maddy-response-source{display:inline-flex;margin:0 7px 7px 0;padding:6px 9px;border:1px solid rgba(105,220,255,.3);border-radius:8px;color:#c9f6ff;text-decoration:none;font-size:.68rem;background:rgba(17,63,91,.45)}
      .meos-maddy-response-actions{display:flex;align-items:center;gap:9px;flex-wrap:wrap;margin-top:18px}.meos-maddy-response-action{border:1px solid rgba(105,220,255,.35);border-radius:9px;padding:8px 12px;background:rgba(18,66,94,.66);color:#e1f8ff;font-size:.72rem;font-weight:800;cursor:pointer}.meos-maddy-response-action[data-primary="true"]{background:linear-gradient(135deg,rgba(17,102,132,.96),rgba(20,78,111,.96));border-color:rgba(111,232,255,.72)}.meos-maddy-response-action:disabled{opacity:.55;cursor:wait}
      .meos-maddy-response-nav{display:flex;align-items:center;gap:8px;margin-left:auto}.meos-maddy-response-position{font-size:.66rem;color:#829eae;min-width:76px;text-align:center}
      @media(max-width:680px){.meos-maddy-response-overlay{padding:8px}.meos-maddy-response-modal{max-height:94vh;border-radius:14px}.meos-maddy-response-head,.meos-maddy-response-body{padding-left:15px;padding-right:15px}.meos-maddy-response-nav{width:100%;margin-left:0;justify-content:space-between}}
...
document
   ...
   * Presentation: one continuous 3:1 Executive Director office panorama.
   * The panorama is scenery only. Every dashboard surface below is a real
   * existing widget DOM node, preserving its data, evidence and click handlers.
   */
  function installImagePanoramicExecutiveOffice(root) {
    if (!root || root.dataset.imagePanoramicOffice === "true") return root;
    const shell = root.querySelector('.meos-dashboard-shell');
    const grid = root.querySelector('#meosWidgetGrid');
    const hero = root.querySelector('.meos-hq-hero');
    const maddy = document.getElementById('meosExecutiveOfficeControl');
    if (!shell || !grid || !hero || !maddy) return root;

    root.dataset.imagePanoramicOffice = 'true';
    document.body.classList.add('meos-image-pano-active');

    const style = document.createElement('style');
    style.id = 'meosImagePanoramicOfficeStyles';
    style.textContent = `
      body.meos-image-pano-active{overflow:hidden!important;background:#050606!important}
      body.meos-image-pano-active [data-meos-navigation]{visibility:hidden!important;pointer-events:none!important}
      #${ROOT_ID}{position:fixed!important;inset:0!important;z-index:9000!important;width:100vw!important;height:100vh!important;overflow:hidden!important;background:#050606!important}
      ...
      .meos-office-pano{position:absolute;inset:0;overflow:hidden;background:#050606;user-select:none;touch-action:pan-y;cursor:grab}
      .meos-office-pano[data-dragging="true"]{cursor:grabbing}
      .meos-office-world{position:absolute;left:50%;top:0;height:100%;transform:translate3d(calc(-50% + var(--office-x,0px)),0,0);transition:transform .48s cubic-bezier(.2,.72,.18,1);will-change:transform;background:linear-gradient(90deg,#030404 0%,#0a0d0d 12%,#111616 23%,#111616 77%,#0a0d0d 88%,#030404 100%)}
      .meos-office-pano[data-dragging="true"] .meos-office-world{transition:none}
      .meos-office-image{position:absolute;top:0;height:100%;width:auto;left:50%;transform:translateX(-50%);display:block;pointer-events:none;-webkit-user-drag:none}
      .meos-office-extension{position:absolute;top:0;bottom:0;pointer-events:none}
      .meos-office-extension.left{left:0;background:linear-gradient(90deg,#030404 0%,rgba(8,11,11,.98) 55%,rgba(10,14,14,.16) 100%)}
      .meos-office-extension.right{right:0;background:linear-gradient(270deg,#030404 0%,rgba(8,11,11,.98) 55%,rgba(10,14,14,.16) 100%)}
      .meos-office-vignette{position:absolute;z-index:5;inset:0;pointer-events:none;background:linear-gradient(90deg,rgba(0,0,0,.38),transparent 5%,transparent 95%,rgba(0,0,0,.38)),linear-gradient(180deg,rgba(0,0,0,.07),transparent 18%,transparent 78%,rgba(0,0,0,.3))}

      /* Peripheral homes: compact executive glance surfaces, not the work area. */
      .meos-office-bay{position:absolute;z-index:20;top:16vh;width:min(360px,22vw);min-width:300px;height:57vh;transform:translateX(-50%);transition:opacity .18s ease,box-shadow .18s ease}
      .meos-office-bay .meos-widget{display:block!important;width:100%!important;height:100%!important;min-height:0!important;overflow:hidden!important;border:1px solid rgba(107,222,231,.20)!important;border-radius:15px!important;background:linear-gradient(145deg,rgba(3,16,21,.25),rgba(5,12,15,.13))!important;box-shadow:0 14px 38px rgba(0,0,0,.20)!important;backdrop-filter:blur(3px)!important}
      ...
      .meos-office-bay .meos-widget-header{position:sticky!important;top:0!important;z-index:4!important;background:linear-gradient(180deg,rgba(4,17,21,.52),rgba(4,17,21,.18))!important;backdrop-filter:blur(5px)!important}
      .meos-office-bay .meos-widget,.meos-office-bay .meos-widget *{text-shadow:0 1px 3px rgba(0,0,0,.82)}
      .meos-office-bay[data-at-desk="true"]{opacity:.28}
      .meos-office-tools{position:absolute;z-index:8;right:8px;top:8px;display:flex;gap:5px}
      .meos-office-tools button{min-height:29px;border:1px solid rgba(214,164,84,.34);border-radius:8px;background:rgba(6,12,14,.88);color:#f0c887;cursor:pointer;padding:0 9px;font:700 .62rem/1 system-ui;letter-spacing:.03em}
      .meos-office-tools button:hover{border-color:rgba(238,195,125,.72);background:rgba(25,22,16,.92)}
      .meos-office-home-marker{display:none;height:100%;place-items:center;border:1px dashed rgba(214,164,84,.28);border-radius:15px;color:rgba(238,195,125,.68);font:700 .66rem/1.3 system-ui;letter-spacing:.08em;text-transform:uppercase;background:rgba(4,10,11,.18)}
      .meos-office-bay[data-at-desk="true"] .meos-office-home-marker{display:grid}
      .meos-office-bay[data-at-desk="true"] .meos-office-tools{display:none}

      /* Center is Maddy + active work. */
      .meos-digital-human-zone{position:absolute;z-index:12;left:50%;top:14vh;width:34vw;height:54vh;transform:translateX(-50%);pointer-events:none}
      .meos-digital-human-zone::before{content:"";position:absolute;inset:0;background:linear-gradient(#9ee7ef,#9ee7ef) left top/38px 1px no-repeat,linear-gradient(#9ee7ef,#9ee7ef) left top/1px 38px no-repeat,linear-gradient(#9ee7ef,#9ee7ef) right top/38px 1px no-repeat,linear-gradient(#9ee7ef,#9ee7ef) right top/1px 38px no-repeat;opacity:.08}
      .meos-digital-human-zone span{position:absolute;left:0;right:0;bottom:3%;text-align:center;color:rgba(239,245,245,.38);font-size:.55rem;letter-spacing:.16em;text-transform:uppercase}

      .meos-desk-workspace{position:absolute;z-index:210;left:50%;top:10vh;width:min(1040px,78vw);height:76vh;transform:translateX(-50%);display:none;border:1px solid rgba(216,171,98,.38);border-radius:18px;background:linear-gradient(145deg,rgba(24,22,18,.90),rgba(9,15,16,.93));box-shadow:0 35px 100px rgba(0,0,0,.68),inset 0 1px 0 rgba(255,255,255,.06);backdrop-filter:blur(18px);overflow:hidden;color:#eef2ef}
      .meos-desk-workspace[data-open="true"]{display:grid;grid-template-rows:auto minmax(0,1fr)}
      .meos-desk-head{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:13px 16px;border-bottom:1px solid rgba(216,171,98,.20);background:linear-gradient(180deg,rgba(55,43,26,.36),rgba(18,20,18,.20))}
      .meos-desk-head-copy{min-width:0}
      .meos-desk-kicker{display:block;color:#d8ad6c;font-size:.57rem;font-weight:800;letter-spacing:.16em;text-transform:uppercase}
      .meos-desk-title{display:block;margin-top:3px;color:#f4f1e9;font-size:.92rem;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
      .meos-desk-actions{display:flex;gap:7px;flex:0 0 auto}
      .meos-desk-actions button{border:1px solid rgba(216,171,98,.34);border-radius:9px;background:rgba(45,36,23,.48);color:#f2d29d;padding:8px 11px;cursor:pointer;font:700 .66rem/1 system-ui}
      .meos-desk-body{min-height:0;padding:14px;overflow:auto}
      .meos-desk-body>.meos-widget{display:block!important;width:100%!important;height:auto!important;min-height:100%!important;grid-column:auto!important;border:0!important;border-radius:12px!important;background:rgba(5,12,13,.30)!important;box-shadow:none!important;overflow:visible!important}
      .meos-desk-body>.meos-widget .meos-widget-inner{height:auto!important;min-height:100%!important;overflow:visible!important;padding:18px!important}
      .meos-desk-body>.meos-widget .meos-widget-header{position:sticky!important;top:-14px!important;z-index:6!important;padding:8px 0!important;background:rgba(12,17,16,.90)!important;backdrop-filter:blur(10px)!important}
      .meos-desk-body a,.meos-desk-body button{position:relative;z-index:2}
      ...
      .meos-office-location{position:absolute;z-index:122;left:50%;bottom:12px;transform:translateX(-50%);padding:6px 10px;border-radius:999px;background:rgba(4,10,12,.46);border:1px solid rgba(213,162,83,.15);color:rgba(235,239,236,.62);font:700 .54rem/1 system-ui;letter-spacing:.07em;text-transform:uppercase;pointer-events:none}

      .meos-desk-maddy-text{position:absolute;z-index:118;left:50%;bottom:5.6%;width:min(560px,39vw);height:76px;transform:translateX(-50%) perspective(800px) rotateX(4deg);border:1px solid rgba(220,177,103,.42);border-radius:13px;background:linear-gradient(180deg,rgba(24,22,18,.70),rgba(7,13,14,.80));box-shadow:0 10px 28px rgba(0,0,0,.34),inset 0 0 0 1px rgba(255,255,255,.025);backdrop-filter:blur(8px);display:grid;grid-template-columns:1fr auto;align-items:center;gap:10px;padding:10px 12px}
      .meos-desk-maddy-text::before{content:"TEXT MADDY";position:absolute;left:13px;top:7px;color:#e6bd7b;font:800 .53rem/1 system-ui;letter-spacing:.13em}
      .meos-desk-maddy-text input{width:100%;height:38px;margin-top:9px;border:1px solid rgba(118,213,224,.26);border-radius:9px;background:rgba(2,9,11,.72);color:#f4f1e9;padding:0 12px;outline:none;font:600 .78rem/1 system-ui;box-shadow:inset 0 0 14px rgba(78,190,207,.045)}
      .meos-desk-maddy-text input::placeholder{color:rgba(227,237,237,.68)}
      .meos-desk-maddy-text input:focus{border-color:rgba(228,183,108,.72);box-shadow:0 0 0 2px rgba(228,183,108,.09),inset 0 0 16px rgba(78,190,207,.055)}
      .meos-desk-maddy-text button{width:42px;height:38px;margin-top:9px;border:1px solid rgba(228,183,108,.46);border-radius:9px;background:linear-gradient(145deg,rgba(77,57,31,.78),rgba(27,23,17,.86));color:#f3d49f;cursor:pointer;font-size:1rem;font-weight:900}
      .meos-desk-maddy-text button:hover{border-color:rgba(241,202,137,.82);background:linear-gradient(145deg,rgba(100,72,35,.88),rgba(35,27,18,.92))}

      .meos-desk-intake{position:absolute;z-index:118;right:5.6%;bottom:6.5%;width:min(255px,19vw);height:92px;border:1px dashed rgba(222,181,111,.48);border-radius:13px;background:linear-gradient(145deg,rgba(22,21,18,.44),rgba(5,12,13,.38));box-shadow:0 9px 24px rgba(0,0,0,.22);backdrop-filter:blur(6px);display:grid;place-items:center;text-align:center;color:#eee5d7;cursor:pointer;padding:10px;transition:border-color .16s ease,background .16s ease,transform .16s ease}
      .meos-desk-intake:hover,.meos-desk-intake[data-dragover="true"]{border-color:rgba(241,201,133,.84);background:linear-gradient(145deg,rgba(55,43,26,.60),rgba(7,15,16,.52));transform:translateY(-2px)}
      .meos-desk-intake strong{display:block;color:#efc985;font:800 .70rem/1.1 system-ui;letter-spacing:.08em;text-transform:uppercase}
      .meos-desk-intake span{display:block;margin-top:6px;color:rgba(236,240,237,.66);font:600 .58rem/1.25 system-ui}
      .meos-desk-intake input{display:none!important}
      .meos-desk-intake-status{position:absolute;left:8px;right:8px;bottom:5px;color:rgba(143,224,197,.82);font:700 .50rem/1 system-ui;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}

      @media(max-width:1100px){
        .meos-office-maddy-dock{left:2.5%;width:210px}
        .meos-desk-maddy-text{width:43vw}
        .meos-desk-intake{right:2.5%;width:205px}
      }

      @media(max-width:900px){
        .meos-office-bay{width:330px;min-width:280px}
        .meos-desk-workspace{width:88vw}
        .meos-office-wayfinding{display:none}
        .meos-office-look.left{left:8px}.meos-office-look.right{right:8px}
      }
    `;
    document.head.appendChild(style);
    ...
    viewport.setAttribute('aria-label', 'Executive Director office. Swipe or drag through the peripheral office. Double-click a widget or choose Send to Desk to work with it.');
    viewport.innerHTML = `
      <div id="meosImageOfficeWorld" class="meos-office-world">
        <div class="meos-office-extension left"></div>
        ...
        <div class="meos-office-extension right"></div>
      ...
      <div id="meosDeskWorkspace" class="meos-desk-workspace" data-open="false" aria-live="polite">
        <header class="meos-desk-head">
          <div class="meos-desk-head-copy"><span class="meos-desk-kicker">Executive Desk · Active Work</span><strong id="meosDeskTitle" class="meos-desk-title">Work Surface</strong></div>
          <div class="meos-desk-actions"><button type="button" data-desk-action="home">Send Home</button></div>
        </header>
        <div id="meosDeskBody" class="meos-desk-body"></div>
      </div>
      <div id="meosOfficeWayfindingLeft" class="meos-office-wayfinding left" aria-label="Left office surfaces"></div>
      <div id="meosOfficeWayfindingRight" class="meos-office-wayfinding right" aria-label="Right office surfaces"></div>
      <div id="meosImageOfficeMaddyDock" class="meos-office-maddy-dock"></div>
      <form id="meosDeskMaddyText" class="meos-desk-maddy-text" aria-label="Text Maddy">
        <input id="meosDeskMaddyInput" type="text" autocomplete="off" placeholder="Talk to Maddy… ask, decide, assign, or work through something." />
        <button type="submit" aria-label="Send to Maddy" title="Send to Maddy">↑</button>
      </form>
      <label id="meosDeskIntake" class="meos-desk-intake" aria-label="Hand files to Maddy">
        <input id="meosDeskIntakeInput" type="file" multiple />
    ...
    const desk = viewport.querySelector('#meosDeskWorkspace');
    const deskBody = viewport.querySelector('#meosDeskBody');
    const deskTitle = viewport.querySelector('#meosDeskTitle');
    const leftWayfinding = viewport.querySelector('#meosOfficeWayfindingLeft');
    const rightWayfinding = viewport.querySelector('#meosOfficeWayfindingRight');

    const widgetIds = ['mission-impact','office-activity','briefing','mission-pulse','priorities','grant-intelligence'];
    const widgetLabels = {
      'mission-impact':'Mission Impact',
      'office-activity':'Office Activity',
      'briefing':'Briefing',
      'mission-pulse':'Mission Pulse',
      'priorities':'Priorities',
      'grant-intelligence':'Resource Acquisition'
    };
    const bays = new Map();
    let deskWidgetId = null;

    widgetIds.forEach(id => {
      const widget = root.querySelector(`.meos-widget[data-widget-id="${id}"]`);
      if (!widget) return;
      const bay = document.createElement('section');
      bay.className = 'meos-office-bay';
      bay.dataset.widget = id;
      bay.dataset.atDesk = 'false';

      const homeMarker = document.createElement('div');
      homeMarker.className = 'meos-office-home-marker';
      homeMarker.textContent = `${widgetLabels[id] || id} · At Desk`;

      const tools = document.createElement('div');
      tools.className = 'meos-office-tools';
      tools.innerHTML = `<button type="button" data-office-tool="desk" title="Send to Desk" aria-label="Send ${widgetLabels[id] || id} to desk">Send to Desk</button>`;

      bay.appendChild(widget);
      bay.appendChild(homeMarker);
      bay.appendChild(tools);
      world.appendChild(bay);
      bays.set(id, { bay, widget, homeMarker });
    });
    ...
    /* File intake delegates through MEOS events so the existing ingestion layer
       remains the authority. We pass the browser File objects; the existing
       document-intake listener can accept/reroute them without a second uploader. */
    const intake = viewport.querySelector('#meosDeskIntake');
    const intakeInput = viewport.querySelector('#meosDeskIntakeInput');
    const intakeStatus = viewport.querySelector('#meosDeskIntakeStatus');
    const handFilesToMaddy = files => {
      const list = Array.from(files || []).filter(Boolean);
      if (!list.length) return;
      if (intakeStatus) intakeStatus.textContent = list.length === 1 ? `Received: ${list[0].name}` : `Received ${list.length} files`;
      dispatchMEOS('meos:document-intake-requested', { files:list, source:'executive-desk', intent:'hand-to-maddy' });
      dispatchMEOS('meos:maddy-files-received', { count:list.length, names:list.map(file => file.name), source:'executive-desk' });
    };
    intakeInput?.addEventListener('change', event => {
      handFilesToMaddy(event.target.files);
      event.target.value = '';
    });
    ['dragenter','dragover'].forEach(type => intake?.addEventListener(type, event => {
      event.preventDefault(); event.stopPropagation(); intake.dataset.dragover = 'true';
    }));
    ['dragleave','drop'].forEach(type => intake?.addEventListener(type, event => {
      event.preventDefault(); event.stopPropagation(); intake.dataset.dragover = 'false';
      if (type === 'drop') handFilesToMaddy(event.dataTransfer?.files);
    }));
    ...
    const setPan = value => {
      ...
      if (location) {
        location.textContent = fraction > .32 ? 'Left · Mission & Organization'
          : fraction < -.32 ? 'Right · Action & Opportunity'
      ...
      if (left) left.disabled = panPx >= maxPanPx - 1;
      if (right) right.disabled = panPx <= -maxPanPx + 1;
    };

    const panToWidget = id => {
      const targetX = bayPositions[id];
      if (!Number.isFinite(targetX)) return;
      const viewportCenterInWorld = worldWidthPx / 2 - panPx;
      const delta = viewportCenterInWorld - targetX;
      setPan(panPx + delta);
    };
    ...
    const sendHome = () => {
      ...
      desk.dataset.open = 'false';
      deskTitle.textContent = 'Work Surface';
    };
    ...
    const addWayfinding = (container, ids) => {
      ids.forEach(id => {
        if (!bays.has(id)) return;
        const button = document.createElement('button');
        button.type = 'button';
        button.textContent = widgetLabels[id] || id;
        button.title = `Go to ${widgetLabels[id] || id}`;
        button.addEventListener('click', () => panToWidget(id));
        container.appendChild(button);
      });
    };
    addWayfinding(leftWayfinding, ['mission-impact','office-activity','briefing']);
    addWayfinding(rightWayfinding, ['mission-pulse','priorities','grant-intelligence']);

    const layoutPanorama = () => {
      const rect = viewport.getBoundingClientRect();
      const height = rect.height || window.innerHeight;
      const width = rect.width || window.innerWidth;
      centerImageWidthPx = height * aspect;

      /* Give MEOS room to grow: each side receives at least 1.45 viewport widths
         beyond the photographed office. The center image is never stretched. */
      extensionPx = Math.max(width * 1.45, 1180);
      worldWidthPx = centerImageWidthPx + extensionPx * 2;
      world.style.width = `${worldWidthPx}px`;

      image.style.width = `${centerImageWidthPx}px`;
      const leftExt = world.querySelector('.meos-office-extension.left');
      const rightExt = world.querySelector('.meos-office-extension.right');
      if (leftExt) leftExt.style.width = `${extensionPx + 80}px`;
      ...
      Object.entries(bayPositions).forEach(([id,x]) => {
        const record = bays.get(id);
        if (record) record.bay.style.left = `${x}px`;
      });

      maxPanPx = Math.max(0, (worldWidthPx - width) / 2);
      setPan(panPx);
    };

    if (image?.naturalWidth && image?.naturalHeight) aspect = image.naturalWidth / image.naturalHeight;
    ...
    viewport.querySelector('.meos-office-look.left')?.addEventListener('click', () => setPan(panPx + Math.max(window.innerWidth * .72, 720)));
    viewport.querySelector('.meos-office-look.right')?.addEventListener('click', () => setPan(panPx - Math.max(window.innerWidth * .72, 720)));
    viewport.addEventListener('keydown', event => {
      if (event.target !== viewport) return;
      if (event.key === 'ArrowLeft') { event.preventDefault(); setPan(panPx + Math.max(window.innerWidth * .55, 560)); }
      else if (event.key === 'ArrowRight') { event.preventDefault(); setPan(panPx - Math.max(window.innerWidth * .55, 560)); }
      else if (event.key === 'Escape' && deskWidgetId) { event.preventDefault(); sendHome(); }
    });
    ...
    viewport.addEventListener('pointermove', event => {
      ...
      dragVelocity = (event.clientX - lastMoveX) / dt;
      lastMoveX = event.clientX;
      lastMoveAt = now;
      setPan(dragPan + (event.clientX - dragX));
    });
    const endDrag = () => {
      if (dragX !== null && Math.abs(dragVelocity) > .08) setPan(panPx + dragVelocity * 240);
      dragX = null;
      viewport.dataset.dragging = 'false';
    };
    ...
    viewport.addEventListener('wheel', event => {
      ...
      event.preventDefault();
      setPan(panPx - horizontalIntent * 1.35);
    }, { passive:false });

    world.addEventListener('click', event => {
      const tool = event.target.closest('[data-office-tool="desk"]');
      if (!tool) return;
      event.preventDefault();
      event.stopPropagation();
      const bay = tool.closest('.meos-office-bay');
      if (bay) sendToDesk(bay.dataset.widget);
    });
    world.addEventListener('dblclick', event => {
      if (event.target.closest('button,a,input,select,textarea')) return;
      const bay = event.target.closest('.meos-office-bay');
      if (bay) {
        event.preventDefault();
        sendToDesk(bay.dataset.widget);
      }
    });
    desk.querySelector('[data-desk-action="home"]')?.addEventListener('click', sendHome);
    ...
    viewport.MEOSOfficeWorkspace = Object.freeze({
      sendToDesk,
      sendHome,
      panToWidget,
      get activeWidgetId() { return deskWidgetId; }
    });
  ...
  function runImagePanoramicExecutiveOfficeAcceptanceTest() {
    ...
    const checks = [
      ...
      ['Briefing real widget preserved', Boolean(scene?.querySelector('.meos-office-bay[data-widget="briefing"] .meos-widget[data-widget-id="briefing"]'))],
      ['Mission Pulse real widget preserved', Boolean(scene?.querySelector('.meos-office-bay[data-widget="mission-pulse"] .meos-widget[data-widget-id="mission-pulse"]'))],
      ['Priorities real widget preserved', Boolean(scene?.querySelector('.meos-office-bay[data-widget="priorities"] .meos-widget[data-widget-id="priorities"]'))],
  ...
  function createDashboardShell() {
    ...
    installMaddyConversationalShell(root);

    return root;
  }


  /* Commission OD4138 — Maddy Conversational Shell Foundation
   *
   * Product direction: conversation first; Canonical Maddy is the presence,
   * MEOS remains the machinery underneath. The panoramic office and permanent
   * widget wall are retired from the primary customer surface. This commission
   * changes presentation only: it reuses canonical request, document intake,
   * Hallway, deliverable, evidence, voice and OD4137A activity truth seams.
   */
  const MADDY_CONVERSATIONAL_SHELL_STYLE_ID = "meosMaddyConversationalShellStyles";

  function injectMaddyConversationalShellStyles() {
    if (document.getElementById(MADDY_CONVERSATIONAL_SHELL_STYLE_ID)) return;
    const style = document.createElement("style");
    style.id = MADDY_CONVERSATIONAL_SHELL_STYLE_ID;
    style.textContent = `
      body.meos-conversation-shell-active{margin:0!important;overflow:hidden!important;background:#07090d!important;color:#f5f6f7!important}
      body.meos-conversation-shell-active [data-meos-navigation]{display:none!important}
      body.meos-conversation-shell-active #meosExecutiveOfficeControl{position:absolute!important;left:-100000px!important;top:-100000px!important;width:1px!important;height:1px!important;overflow:hidden!important;opacity:0!important;pointer-events:none!important}
      #${ROOT_ID}{position:fixed!important;inset:0!important;z-index:9000!important;width:100vw!important;height:100dvh!important;min-height:100vh!important;overflow:hidden!important;background:#07090d!important}
      #${ROOT_ID} .meos-dashboard-shell{position:absolute!important;left:-100000px!important;top:-100000px!important;width:1px!important;height:1px!important;overflow:hidden!important;opacity:0!important;pointer-events:none!important}

      .meos-maddy-shell{--maddy-accent:#7bdff0;--maddy-warm:#d6ad73;position:absolute;inset:0;display:grid;grid-template-rows:auto minmax(0,1fr) auto;background:radial-gradient(circle at 50% -12%,rgba(54,91,114,.24),transparent 38%),linear-gradient(180deg,#0a0d12 0%,#07090d 52%,#080a0e 100%);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#f4f7f8;isolation:isolate}
      .meos-maddy-shell[data-mode="personal"]{--maddy-accent:#a7e4df;--maddy-warm:#e2b78b;background:radial-gradient(circle at 50% -10%,rgba(97,76,107,.28),transparent 40%),linear-gradient(180deg,#100e14 0%,#090a0e 55%,#08090c 100%)}
      ...
      .meos-maddy-conversation{min-height:0;overflow:auto;scrollbar-width:thin;scrollbar-color:rgba(255,255,255,.14) transparent;padding:28px max(22px,5vw) 34px;overscroll-behavior:contain}
      .meos-maddy-conversation-inner{width:min(780px,100%);margin:0 auto;display:grid;gap:24px}
      .meos-maddy-empty{color:rgba(228,234,237,.62);font-size:1.05rem;line-height:1.65;padding:8px 0 18px}
      .meos-maddy-message{display:grid;gap:7px;font-size:1.08rem;line-height:1.7;letter-spacing:-.006em}.meos-maddy-message-role{font-size:.8rem;line-height:1;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:rgba(220,228,232,.48)}
      .meos-maddy-message[data-role="user"]{margin-left:min(18%,120px);padding:13px 16px;border-radius:20px;background:rgba(255,255,255,.075);border:1px solid rgba(255,255,255,.065)}
  ...
  function appendMaddyShellMessage(role, text, options = {}) {
    const stream = document.getElementById("meosMaddyShellMessages");
    if (!stream) return null;
    const value = String(text || "").trim();
    if (!value) return null;
    document.getElementById("meosMaddyShellEmpty")?.remove();
    const article = document.createElement("article");
    article.className = "meos-maddy-message";
    article.dataset.role = role === "user" ? "user" : "maddy";
    if (options.messageId) article.dataset.messageId = String(options.messageId);
    const label = document.createElement("div");
    label.className = "meos-maddy-message-role";
    label.textContent = role === "user" ? "You" : "Maddy";
    const body = document.createElement("div");
    body.className = "meos-maddy-message-body";
    body.textContent = value;
    article.append(label, body);
    const urls = Array.isArray(options.sources) ? options.sources.filter(Boolean).slice(0,8) : [];
    if (urls.length) {
      const sources = document.createElement("div");
      sources.className = "meos-maddy-message-sources";
      urls.forEach((url, index) => {
        const link = document.createElement("a");
        link.href = url; link.target = "_blank"; link.rel = "noopener noreferrer";
        link.textContent = sourceLabelFromUrl(url) || `Source ${index + 1}`;
        sources.appendChild(link);
      });
      article.appendChild(sources);
    }
    stream.appendChild(article);
    const conversation = document.getElementById("meosMaddyConversation");
    if (conversation) conversation.scrollTop = conversation.scrollHeight;
    return article;
  }

  function handConversationalShellFilesToMaddy(files) {
    const list = Array.from(files || []).filter(Boolean);
    if (!list.length) return false;
    const note = document.getElementById("meosMaddyShellFileNote");
    if (note) note.textContent = list.length === 1 ? `Attached ${list[0].name}` : `Attached ${list.length} files`;
    dispatchMEOS("meos:document-intake-requested", { files:list, source:"maddy-conversational-shell", intent:"hand-to-maddy" });
    dispatchMEOS("meos:maddy-files-received", { count:list.length, names:list.map(file => file.name), source:"maddy-conversational-shell" });
    return true;
  }

  function renderMaddyNowSurface(snapshot = null) {
    const found = document.getElementById("meosMaddyFoundList");
    const approvals = document.getElementById("meosMaddyApprovalList");
    if (!found || !approvals) return false;
    const live = snapshot || collectHeadquartersSnapshot();
    const deliverables = Array.isArray(live.hallwayDeliverables) ? live.hallwayDeliverables.slice(0,6) : [];
    const pending = Array.isArray(live.pendingApprovals) ? live.pendingApprovals.slice(0,8) : [];
    const foundCount = document.getElementById("meosMaddyFoundCount");
    const approvalCount = document.getElementById("meosMaddyApprovalCount");
    const nowButton = document.getElementById("meosMaddyNowButton");
    if (foundCount) foundCount.textContent = String(deliverables.length);
    ...
    if (!deliverables.length) {
      const empty = document.createElement("div"); empty.className = "meos-maddy-now-empty"; empty.textContent = "Nothing new returned yet."; found.appendChild(empty);
    } else {
      deliverables.forEach((deliverable) => {
        const view = getMaddyDeliverablePresentation(deliverable, 0, 1);
        const item = document.createElement("article"); item.className = "meos-maddy-now-item";
        const title = document.createElement("strong"); title.textContent = decodeMaddyText(view.title || deliverable.title || "Maddy found something"); item.appendChild(title);
        const summary = document.createElement("p"); summary.textContent = decodeMaddyText(view.summary || "Returned work is ready to inspect."); item.appendChild(summary);
        const actions = document.createElement("div"); actions.className = "meos-maddy-now-actions";
        const details = document.createElement("button"); details.type = "button"; details.textContent = "See what Maddy found"; details.addEventListener("click", () => openReturnedWorkPackage(deliverable)); actions.appendChild(details);
        if (view.sourceUrl) { const source=document.createElement("a"); source.href=view.sourceUrl; source.target="_blank"; source.rel="noopener noreferrer"; source.textContent="Source ↗"; actions.appendChild(source); }
        item.appendChild(actions); found.appendChild(item);
      });
    }

    approvals.innerHTML = "";
    if (!pending.length) {
      const empty = document.createElement("div"); empty.className = "meos-maddy-now-empty"; empty.textContent = "No approval is waiting on you."; approvals.appendChild(empty);
    } else {
      pending.forEach((approval) => {
        const item = document.createElement("article"); item.className = "meos-maddy-now-item meos-maddy-approval-item";
        const title = document.createElement("strong"); title.textContent = approval.title || "Maddy needs your approval"; item.appendChild(title);
        const summary = document.createElement("p"); summary.textContent = `${approval.officeName || "Maddy"} reached an authority boundary and is waiting for you.`; item.appendChild(summary);
        const actions = document.createElement("div"); actions.className = "meos-maddy-now-actions";
        if (approval.source === "executive-hallway" && Array.isArray(approval.options) && approval.options.includes("take-it")) {
          const take = document.createElement("button"); take.type = "button"; take.dataset.primary = "true"; take.textContent = "Take It — Approve";
          take.addEventListener("click", async () => {
            const hallway = getExecutiveHallway(); if (!hallway?.takeIt || !approval.id) return;
            take.disabled = true; take.textContent = "Approved · Maddy working…";
            try { await hallway.takeIt(approval.id, { signal:"Take It!", source:"maddy-conversational-shell" }); }
            catch (error) { take.disabled = false; take.textContent = "Take It — Approve"; state.hallway.lastError = error?.message || String(error); renderMaddyConversationalShell(); }
          });
          actions.appendChild(take);
        }
        const evidence = document.createElement("button"); evidence.type = "button"; evidence.textContent = "Why approval?"; evidence.addEventListener("click", () => openRealtimeEvidence("approvals")); actions.appendChild(evidence);
        item.appendChild(actions); approvals.appendChild(item);
      });
    }
    return true;
  }
  ...
  function renderMaddyConversationalShell(activityModel = null) {
    ...
    const model = activityModel || getMaddyActivityModel();
    shell.dataset.mode = state.communicationMode || "professional";
    const mode = document.getElementById("meosMaddyShellMode");
    ...
    if (model.answer && model.answerDeliverable) {
      ...
      if (deliverableId && shell.dataset.lastAnswerDeliverable !== deliverableId) {
        appendMaddyShellMessage("maddy", model.answer, { messageId:`deliverable:${deliverableId}`, sources:model.sources });
        shell.dataset.lastAnswerDeliverable = deliverableId;
      }
    }
  ...
  function installMaddyConversationalShell(root) {
    if (!root || root.dataset.maddyConversationalShell === "true") return root;
    const legacyShell = root.querySelector(".meos-dashboard-shell");
    if (!legacyShell) return root;
    root.dataset.maddyConversationalShell = "true";
    root.dataset.primaryExperience = "conversation";
    document.body.classList.remove("meos-image-pano-active");
    document.body.classList.add("meos-conversation-shell-active");
    injectMaddyConversationalShellStyles();

    const shell = document.createElement("section");
    shell.id = "meosMaddyConversationalShell";
    ...
    shell.innerHTML = `
      ...
      <footer class="meos-maddy-composer-wrap">
        <form id="meosMaddyShellComposer" class="meos-maddy-composer">
          <button id="meosMaddyShellAttach" class="meos-maddy-compose-icon" type="button" aria-label="Attach files" title="Attach files">+</button>
          <textarea id="meosMaddyShellInput" rows="1" autocomplete="off" placeholder="Message Maddy…" aria-label="Message Maddy"></textarea>
          <button id="meosMaddyShellVoice" class="meos-maddy-compose-icon" type="button" aria-label="Talk to Maddy" title="Talk to Maddy">◉</button>
          <button class="meos-maddy-compose-icon meos-maddy-compose-send" type="submit" aria-label="Send to Maddy" title="Send">↑</button>
          <input id="meosMaddyShellFiles" type="file" multiple hidden />
        </form>
        <div id="meosMaddyShellFileNote" class="meos-maddy-file-note" aria-live="polite"></div>
      </footer>`;
    root.appendChild(shell);

    const form = shell.querySelector("#meosMaddyShellComposer");
    const input = shell.querySelector("#meosMaddyShellInput");
    form?.addEventListener("submit", (event) => {
      event.preventDefault();
      const message = String(input?.value || "").trim();
      if (!message) { input?.focus(); return; }
      appendMaddyShellMessage("user", message);
      dispatchExecutiveDeskTextCommand(message);
      input.value = "";
    ...
    const fileInput = shell.querySelector("#meosMaddyShellFiles");
    shell.querySelector("#meosMaddyShellAttach")?.addEventListener("click", () => fileInput?.click());
    fileInput?.addEventListener("change", (event) => { handConversationalShellFilesToMaddy(event.target.files); event.target.value = ""; });
    shell.addEventListener("dragover", (event) => { if (event.dataTransfer?.types?.includes("Files")) event.preventDefault(); });
    shell.addEventListener("drop", (event) => { if (!event.dataTransfer?.files?.length) return; event.preventDefault(); handConversationalShellFilesToMaddy(event.dataTransfer.files); });

    shell.querySelector("#meosMaddyShellVoice")?.addEventListener("click", toggleVoiceConnection);
    ...
    window.addEventListener("meos:maddy:response", (event) => {
      ...
      const text = String(detail.text || detail.answer || detail.message || detail.response || "").trim();
      if (!text) return;
      const responseId = String(detail.turnId || detail.id || `${Date.now()}`);
      if (shell.querySelector(`[data-message-id="response:${CSS.escape(responseId)}"]`)) return;
      appendMaddyShellMessage("maddy", text, { messageId:`response:${responseId}` });
    });
    window.addEventListener("meos:maddy-mode-changed", () => renderMaddyConversationalShell());

    renderMaddyConversationalShell();
    return root;
  }

  function runMaddyConversationalShellAcceptanceTest() {
    const shell = document.getElementById("meosMaddyConversationalShell");
    ...
    const fileSource = handConversationalShellFilesToMaddy.toString();
    const nowSource = renderMaddyNowSurface.toString();
    const checks = [
      { name:"OD4138 has a dedicated build identity", passed:MADDY_CONVERSATIONAL_SHELL_BUILD_ID === "OD4138-MADDY-CONVERSATIONAL-SHELL-FOUNDATION-20260921-A" },
      { name:"Conversation shell is the primary experience", passed:Boolean(shell && document.getElementById(ROOT_ID)?.dataset?.primaryExperience === "conversation") },
      { name:"Primary UI no longer depends on panoramic office", passed:Boolean(shell) && !document.body.classList.contains("meos-image-pano-active") && !document.getElementById("meosImagePanoramicOffice") },
      { name:"Canonical Maddy v2 has a dedicated presence area", passed:shell?.querySelector('.meos-maddy-portrait')?.getAttribute('src') === 'maddy-canonical-v2.png' && Boolean(shell?.querySelector('#meosMaddyStage')) },
      { name:"Readable conversation typography is at least roughly 17px desktop", passed:/font-size:1\.08rem/.test(styleText) && /line-height:1\.7/.test(styleText) },
      { name:"Mobile shell is explicit and removes the permanent right rail", passed:/@media\(max-width:900px\)/.test(styleText) && /translateX\(105%\)/.test(styleText) },
      { name:"Composer dispatches through canonical Maddy request seam", passed:/dispatchExecutiveDeskTextCommand\(message\)/.test(installerSource) },
      { name:"Composer-integrated files reuse canonical document intake authority", passed:/meos:document-intake-requested/.test(fileSource) && !/fetch\(/.test(fileSource) },
      { name:"Truthful OD4137A activity model drives stage status", passed:/getMaddyActivityModel/.test(renderMaddyConversationalShell.toString()) && /model\.label/.test(renderMaddyConversationalShell.toString()) },
      { name:"Activity is subtitle-like rather than the old boxed work panel", passed:Boolean(shell?.querySelector('#meosMaddyStageStatus')) && !shell?.querySelector('.meos-live-activity') },
      { name:"Subtle left-to-right activity sweep is present", passed:/@keyframes meosMaddySweep/.test(styleText) && /translateX\(430%\)/.test(styleText) },
      { name:"Reduced-motion behavior is explicit", passed:/prefers-reduced-motion:reduce/.test(styleText) },
      { name:"Maddy Found is backed by Hallway deliverables", passed:/hallwayDeliverables/.test(nowSource) && /getMaddyDeliverablePresentation/.test(nowSource) },
      { name:"Needs your approval is backed by pending approval truth", passed:/pendingApprovals/.test(nowSource) && /await/.test(nowSource) },
      { name:"Approval action reuses existing Hallway Take It authority", passed:/hallway\.takeIt/.test(nowSource) && !/submitWork/.test(nowSource) },
      { name:"Shell adds no provider, spend, retry, or external-action authority", passed:!/(automaticSpend|providerDisplay|response\.create|submitWork|fetch\()/i.test(`${installerSource} ${fileSource}`) },
      { name:"No permanent widget grid is part of the primary shell", passed:Boolean(shell) && !shell.querySelector('.meos-widget') && !shell.querySelector('#meosWidgetGrid') },
      { name:"Desktop retains visible Findings and Approval context without modal hunting", passed:Boolean(shell?.querySelector('#meosMaddyFoundList') && shell?.querySelector('#meosMaddyApprovalList')) }
    ];
    const passed = checks.filter((check) => check.passed).length;
  ...
  /* Commission 006.033B — Conversational Live Maddy Workstream
   *
   * One Maddy. One conversation. One place to ask, watch truthful work, authorize,
   * and receive the answer. This remains a projection of canonical Hallway truth;
   * it creates no second work/cognition engine and performs no paid display calls.
   */
  const MADDY_ACTIVITY_SCHEMA = "meos.dashboard.maddy-conversational-workstream.v1";
  ...
  function injectMaddyActivityStyles() {
    ...
    style.textContent = `
      .meos-live-activity{position:relative;z-index:2;width:100%;margin:0 0 9px;border:1px solid rgba(105,239,255,.18);border-radius:14px;background:linear-gradient(145deg,rgba(4,15,27,.82),rgba(9,27,42,.72));box-shadow:0 12px 34px rgba(0,0,0,.18),0 0 22px rgba(105,239,255,.045);color:#eafaff;overflow:hidden;display:none}
      .meos-live-activity[data-visible="true"]{display:block}
      .meos-live-activity-main{display:grid;grid-template-columns:auto 1fr auto;gap:10px;align-items:center;padding:10px 11px}
      .meos-live-activity-orb{width:26px;height:26px;border-radius:50%;border:1px solid rgba(105,239,255,.38);background:radial-gradient(circle at 35% 30%,#eaffff 0 5%,#69efff 7%,#3269b5 27%,#071321 65%);box-shadow:0 0 14px rgba(105,239,255,.2);position:relative}
      .meos-live-activity[data-active="true"] .meos-live-activity-orb::after{content:"";position:absolute;inset:-4px;border:1px solid rgba(105,239,255,.28);border-radius:50%;animation:meosActivityPulse 1.45s ease-out infinite}
      @keyframes meosActivityPulse{0%{transform:scale(.84);opacity:.9}100%{transform:scale(1.38);opacity:0}}
      .meos-live-activity-copy{min-width:0}.meos-live-activity-copy strong{display:block;font-size:.78rem;letter-spacing:.01em}.meos-live-activity-copy span{display:block;margin-top:2px;color:rgba(205,236,246,.7);font-size:.66rem;line-height:1.35;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
      .meos-live-activity-toggle{border:0;background:transparent;color:rgba(210,244,252,.72);font-size:.62rem;cursor:pointer;padding:7px;border-radius:8px}.meos-live-activity-toggle:hover{background:rgba(105,239,255,.08);color:#fff}
      .meos-live-activity-detail{display:none;padding:0 11px 11px;border-top:1px solid rgba(105,239,255,.08)}.meos-live-activity[data-expanded="true"] .meos-live-activity-detail{display:block}
      .meos-live-activity-steps{display:flex;gap:4px;flex-wrap:wrap;padding-top:9px}.meos-live-activity-step{padding:4px 7px;border-radius:999px;border:1px solid rgba(160,194,210,.12);font-size:.54rem;color:rgba(201,225,235,.55)}.meos-live-activity-step[data-current="true"]{border-color:rgba(105,239,255,.34);color:#c9f9ff;background:rgba(105,239,255,.07)}.meos-live-activity-step[data-done="true"]{color:#91e9bd;border-color:rgba(92,224,164,.2)}
      .meos-live-activity-truth{margin-top:8px;font-size:.62rem;line-height:1.45;color:rgba(205,232,242,.7)}.meos-live-activity-truth b{color:#effcff}.meos-live-activity-branches{margin-top:6px;color:rgba(169,210,225,.65);font-size:.59rem}
      .meos-live-workstream-events{display:grid;gap:5px;margin-top:8px}.meos-live-workstream-event{display:flex;gap:7px;align-items:flex-start;font-size:.6rem;line-height:1.4;color:rgba(203,229,239,.72)}.meos-live-workstream-event::before{content:"·";color:#69efff}
  ...
  function getMaddyActivityModel(snapshot = null) {
    ...
    const deliverables = Array.isArray(hallway?.deliverables) ? hallway.deliverables : [];
    const history = Array.isArray(hallway?.history) ? hallway.history : [];
    ...
    const relatedDeliverables = primary?.id ? deliverables.filter((item)=>item?.workId===primary.id) : [];
  ...
  function renderMaddyActivitySurface(snapshot = null) {
    ...
    if(model.answer){ const a=document.createElement("div"); a.className="meos-live-workstream-answer"; a.textContent=model.answer; answerNode.appendChild(a); if(model.sources.length){ const sw=document.createElement("div"); sw.className="meos-live-workstream-sources"; model.sources.forEach((url,i)=>{const link=document.createElement("a");link.className="meos-live-workstream-source";link.href=url;link.target="_blank";link.rel="noopener noreferrer";link.textContent=sourceLabelFromUrl(url)||`Source ${i+1}`;sw.appendChild(link)});answerNode.appendChild(sw); } }
    const actions=panel.querySelector("#meosMaddyWorkstreamActions"); actions.innerHTML="";
    if(model.needsApproval && model.options.includes("take-it")){ const take=document.createElement("button");take.type="button";take.className="meos-live-workstream-action";take.dataset.primary="true";take.textContent="Take It — Authorize";take.addEventListener("click",async()=>{const hallway=getExecutiveHallway();if(!hallway?.takeIt||!model.workId)return;take.disabled=true;take.textContent="Authorized · Maddy working…";try{await hallway.takeIt(model.workId,{signal:"Take It!",source:"maddy-conversational-workstream"})}catch(error){state.hallway.lastError=error?.message||String(error);renderMaddyActivitySurface()}});actions.appendChild(take); }
    if(model.answerDeliverable){ const evidence=document.createElement("button");evidence.type="button";evidence.className="meos-live-workstream-action";evidence.textContent="Evidence & Details";evidence.addEventListener("click",()=>openReturnedWorkPackage(model.answerDeliverable));actions.appendChild(evidence); }
    renderMaddyConversationalShell(model);
    return model;
  }

  function runMaddyActivitySurfaceAcceptanceTest() {
    const panel=ensureMaddyActivitySurface();
    const fixture=(stateValue,extra={})=>({work:[{id:"workstream-test",state:stateValue,title:"Find the best path",owner:"Maddy",updatedAt:new Date().toISOString(),...extra}],deliverables:[],history:[]});
    const thinking=getMaddyActivityModel({hallway:fixture("understanding")});
    const approval=getMaddyActivityModel({hallway:fixture("awaiting-review",{options:["take-it"]})});
    const doneFixture=fixture("done"); doneFixture.deliverables=[{id:"answer-test",workId:"workstream-test",kind:"research-status",title:"Answer",data:{answer:"The governed answer is here."}}];
    const done=getMaddyActivityModel({hallway:doneFixture});
    const staleFixture=fixture("executing",{updatedAt:new Date(Date.now()-60000).toISOString()});
    ...
    const checks=[
      ...
      ["Done state can carry the returned Maddy answer",done.state==="done" && done.answer==="The governed answer is here."],
      ["Answer remains bound to the same work lineage",done.answerDeliverable?.workId===done.workId],
      ["Evidence/details remains separate from answer text",typeof openReturnedWorkPackage==="function"],
      ["Recorded milestones come from Hallway history",/hallway\.history/.test(String(getMaddyActivityModel))],
      ["Staleness is detected from recorded timestamps",stale.stale===true],
      ["Staleness refuses fake heartbeat/progress",stale.truth.includes("not inventing a heartbeat")],
      ["No fake percentage exists in workstream model",!JSON.stringify(thinking).includes("percent")],
      ["Rendering requires zero paid cognition calls",thinking.paidDisplayRequests===0],
      ["Rendering requires zero provider calls",thinking.providerDisplayRequests===0],
      ["Canonical Hallway snapshot remains source of work truth",/getHallwaySnapshot/.test(String(getMaddyActivityModel))],
      ["Existing Maddy dispatch presentation remains reused",/getMaddyDispatchPresentation/.test(String(getMaddyActivityModel))],
      ["No second work store is introduced",!String(getMaddyActivityModel).includes("localStorage")],
      ["Answer uses existing governed deliverable answer path",/maddyDeliverableAnswerText/.test(String(getMaddyActivityModel))],
      ["Sources use existing strict governed source path",/strictGovernedSourceUrls/.test(String(getMaddyActivityModel))],
      ["Surface can expand upward without replacing digital-human stage",panel.tagName==="SECTION" && Boolean(document.querySelector(".meos-maddy-field"))],
      ["Reduced-motion support remains present",document.getElementById(MADDY_ACTIVITY_STYLE_ID)?.textContent.includes("prefers-reduced-motion")===true],
      ["Mobile presentation remains explicit",document.getElementById(MADDY_ACTIVITY_STYLE_ID)?.textContent.includes("max-width:600px")===true],
      ["One conversational surface owns work and returned answer",Boolean(panel.querySelector("#meosMaddyWorkstreamAnswer")&&panel.querySelector("#meosMaddyWorkstreamActions"))],
  ...
  function handleHallwayWorkUpdated(event) {
    ...
    // 006.033B: authority, live work, blockers, and completion remain in the same
    // conversational workstream above the Ask Maddy input. The legacy modal remains
    // available through its public helpers, but Hallway events no longer force it open.
  }
  ...
  function handleHallwayDeliverableReady(event) {
    ...
    renderLiveHeadquarters();
    renderMaddyActivitySurface();
    // 006.033B: the returned answer is rendered inline by renderMaddyActivitySurface().
  }
  ...
  function bindHallwayEvents() {
    ...
    if (latestWork) handleHallwayWorkUpdated({ detail: latestWork, initial: true });
    if (latestDeliverable) handleHallwayDeliverableReady({ detail: latestDeliverable, initial: true });
  }
  ...
  function ensureMaddyResponseModal() {
    let overlay = document.getElementById("meosMaddyResponseModal");
    if (overlay) return overlay;

    overlay = document.createElement("div");
    overlay.id = "meosMaddyResponseModal";
    overlay.className = "meos-maddy-response-overlay";
    overlay.dataset.open = "false";
    overlay.setAttribute("role", "dialog");
    overlay.setAttribute("aria-modal", "true");
    overlay.setAttribute("aria-labelledby", "meosMaddyResponseTitle");
    overlay.innerHTML = `
      <section class="meos-maddy-response-modal">
        <header class="meos-maddy-response-head">
          <div>
            <div id="meosMaddyResponseKicker" class="meos-maddy-response-kicker">Maddy</div>
            <h2 id="meosMaddyResponseTitle" class="meos-maddy-response-title">Response</h2>
          </div>
          <button id="meosMaddyResponseClose" class="meos-maddy-response-close" type="button" aria-label="Close Maddy response">×</button>
        </header>
        <div id="meosMaddyResponseBody" class="meos-maddy-response-body"></div>
      </section>`;
    overlay.addEventListener("click", (event) => { if (event.target === overlay) closeMaddyResponseModal(); });
    overlay.querySelector("#meosMaddyResponseClose")?.addEventListener("click", closeMaddyResponseModal);
    document.body.appendChild(overlay);
    return overlay;
  }

  function closeMaddyResponseModal() {
    const overlay = document.getElementById("meosMaddyResponseModal");
    if (!overlay) return false;
    overlay.dataset.open = "false";
    overlay.dataset.workId = "";
    overlay.dataset.deliverableId = "";
    return true;
  }

  function compactMaddyResponseTitle(value, fallback = "Maddy's response") {
    const text = decodeMaddyText(String(value || "").replace(/^Executive Outcome\s*[—-]\s*/i, "").trim());
    if (!text) return fallback;
    return text.length > 118 ? `${text.slice(0, 115).trim()}…` : text;
  }

  function maddyDeliverableAnswerText(deliverable) {
    const data = deliverable?.data || {};
    const governed = getGovernedAnswerFromDeliverable(deliverable);
    return decodeMaddyText(firstBriefValue(
      governed?.answer,
      data.answer,
      data.finding,
      data.conclusion,
      ...
      data.resourceDevelopment?.executiveBrief?.summary,
      deliverable?.summary
    ) || "").trim();
  }

  function openMaddyResponseForDeliverable(deliverable, options = {}) {
    if (!deliverable) return false;
    const overlay = ensureMaddyResponseModal();
    const body = overlay.querySelector("#meosMaddyResponseBody");
    const kicker = overlay.querySelector("#meosMaddyResponseKicker");
    const title = overlay.querySelector("#meosMaddyResponseTitle");
    if (!body || !kicker || !title) return false;

    const snapshot = collectHeadquartersSnapshot();
    const siblings = (snapshot.hallwayDeliverables || []).filter((item) => item.workId && item.workId === deliverable.workId);
    const items = siblings.length ? siblings : [deliverable];
    let index = Math.max(0, items.findIndex((item) => item.id === deliverable.id));
    const selected = items[index] || deliverable;
    const view = getMaddyDeliverablePresentation(selected, index, items.length);
    const answer = maddyDeliverableAnswerText(selected) || decodeMaddyText(view.summary || "").trim();
    const sources = strictGovernedSourceUrls(selected);
    if (!sources.length && view.sourceUrl) sources.push(view.sourceUrl);

    kicker.textContent = view.type === "opportunity" ? "Maddy · Funding Result" : view.type === "research" ? "Maddy · Research Answer" : "Maddy's Answer";
    title.textContent = compactMaddyResponseTitle(view.title, view.type === "opportunity" ? "Funding opportunity" : "Maddy's response");
    body.innerHTML = "";

    const meta = document.createElement("div"); meta.className = "meos-maddy-response-meta";
    const typeChip = document.createElement("span"); typeChip.className = "meos-maddy-response-chip"; typeChip.textContent = view.type === "opportunity" ? "Funding" : view.type === "research" ? "Research / Learning" : "Executive Work"; meta.appendChild(typeChip);
    if (items.length > 1) { const countChip=document.createElement("span"); countChip.className="meos-maddy-response-chip"; countChip.textContent=`${index + 1} of ${items.length}`; meta.appendChild(countChip); }
    body.appendChild(meta);

    const answerNode = document.createElement("div"); answerNode.className = "meos-maddy-response-answer";
    answerNode.textContent = answer || "Maddy returned a result, but no human-readable answer text was included in this deliverable.";
    body.appendChild(answerNode);

    if (sources.length) {
      const sourceWrap = document.createElement("div"); sourceWrap.className = "meos-maddy-response-sources";
      const sourceLabel = document.createElement("span"); sourceLabel.className = "meos-maddy-response-sources-label"; sourceLabel.textContent = "Sources / evidence"; sourceWrap.appendChild(sourceLabel);
      sources.slice(0, 8).forEach((url, sourceIndex) => {
        const link = document.createElement("a"); link.className = "meos-maddy-response-source"; link.href = url; link.target = "_blank"; link.rel = "noopener noreferrer"; link.textContent = sourceLabelFromUrl(url) || `Source ${sourceIndex + 1}`; sourceWrap.appendChild(link);
      });
    ...
    if (items.length > 1) {
      ...
      const position = document.createElement("span"); position.className="meos-maddy-response-position"; position.textContent=`${index + 1} / ${items.length}`;
      const next = document.createElement("button"); next.type="button"; next.className="meos-maddy-response-action"; next.textContent="Next →"; next.disabled=index===items.length-1; next.addEventListener("click",()=>openMaddyResponseForDeliverable(items[index+1], { navigation: true }));
      nav.append(previous, position, next); actions.appendChild(nav);
    }
    body.appendChild(actions);

    overlay.dataset.workId = selected.workId || "";
    overlay.dataset.deliverableId = selected.id || "";
    overlay.dataset.mode = "answer";
    overlay.dataset.open = "true";
    state.hallway.selectedDeliverableId = selected.id || state.hallway.selectedDeliverableId;
    if (!options.navigation) overlay.querySelector(".meos-maddy-response-modal")?.scrollTo?.({ top: 0 });
    return true;
  }

  function openMaddyResponseForWork(work) {
    ...
    const dispatch = getMaddyDispatchPresentation(work);
    const stateName = String(work.state || "received");
    const awaitingReview = stateName === "awaiting-review";
    const failed = ["blocked", "failed"].includes(stateName);
    kicker.textContent = awaitingReview ? "Maddy · Authorization Required" : failed ? "Maddy · Work Stopped" : "Maddy · Working";
    title.textContent = awaitingReview ? "I need your decision before I can answer" : failed ? "I couldn't complete this assignment" : dispatch.label;
    body.innerHTML = "";

    const status = document.createElement("p"); status.className = "meos-maddy-response-status";
    status.textContent = awaitingReview
      ? "I have your assignment, but I have not run the research yet. The text below is your question/assignment — not my answer. Authorize the work and I can continue."
      : failed
        ? (work?.error ? `The assignment stopped: ${String(work.error?.message || work.error)}` : dispatch.detail)
        : dispatch.detail;
    body.appendChild(status);

    const assignment = document.createElement("div"); assignment.className = "meos-maddy-response-assignment";
    const assignmentLabel = document.createElement("strong"); assignmentLabel.textContent = "Your assignment";
    const assignmentText = document.createElement("span"); assignmentText.textContent = decodeMaddyText(work.instruction || work.title || "Executive work");
    assignment.append(assignmentLabel, assignmentText); body.appendChild(assignment);

    const actions = document.createElement("div"); actions.className = "meos-maddy-response-actions";
    if (awaitingReview && Array.isArray(work.options) && work.options.includes("take-it")) {
      const takeIt = document.createElement("button"); takeIt.type = "button"; takeIt.className = "meos-maddy-response-action"; takeIt.dataset.primary = "true"; takeIt.textContent = "Take It — Authorize";
      takeIt.addEventListener("click", async () => {
        const hallway = getExecutiveHallway();
        if (!hallway?.takeIt || !work.id) return;
        takeIt.disabled = true; takeIt.textContent = "Authorized · Maddy working…";
        try { await hallway.takeIt(work.id, { signal: "Take It!", source: "maddy-response-surface" }); }
        catch (error) { openMaddyResponseForWork({ ...work, state: "failed", error: error?.message || String(error) }); }
      });
      actions.appendChild(takeIt);
    }
    const close = document.createElement("button"); close.type="button"; close.className="meos-maddy-response-action"; close.textContent="Close"; close.addEventListener("click",closeMaddyResponseModal); actions.appendChild(close);
    body.appendChild(actions);

    overlay.dataset.workId = work.id || "";
    overlay.dataset.deliverableId = "";
    overlay.dataset.mode = awaitingReview ? "authorization" : failed ? "failure" : "working";
    overlay.dataset.open = "true";
    return true;
  }

  function runMaddyResponseSurfaceAcceptanceTest() {
    const overlay = ensureMaddyResponseModal();
    const answerFixture = {
      id: "maddy-response-surface-answer", workId: "maddy-response-surface-work", kind: "research-status", title: "CCSP funding research",
      summary: "Returned summary should not override the governed answer.",
      data: { governedAnswer: { answer: "Maddy found a verified local funding lead.", citations: ["https://example.org/funding"] } }
    };
    const answer = maddyDeliverableAnswerText(answerFixture);
    const workFixture = { id: "maddy-response-surface-review", state: "awaiting-review", instruction: "Find CCSP money", options: ["take-it"], authority: { reviewRequired: true, authorized: false } };
    const checks = [
      { name: "Dedicated Maddy response modal can be created", passed: Boolean(overlay?.querySelector(".meos-maddy-response-modal")) },
      { name: "Governed answer wins over assignment/summary text", passed: answer === "Maddy found a verified local funding lead." },
      { name: "Authorization work is distinguishable from an answer", passed: getMaddyDispatchPresentation(workFixture).state === "awaiting-review" },
      { name: "Authorization preserves Take It instead of inventing execution authority", passed: workFixture.options.includes("take-it") && workFixture.authority.authorized === false },
      { name: "Answer surface has source/evidence navigation", passed: typeof renderMaddyExecutiveBrief === "function" && strictGovernedSourceUrls(answerFixture)[0] === "https://example.org/funding" },
      { name: "Multiple returned results can be navigated without changing Hallway authority", passed: typeof openMaddyResponseForDeliverable === "function" },
      { name: "No second conversation transport is introduced", passed: typeof submitMaddyRequest === "function" && typeof getExecutiveHallway === "function" }
    ];
    const passed = checks.filter((check) => check.passed).length;
    const result = { success: passed === checks.length, commission: "MADDY-RESPONSE-SURFACE", schema: "meos.dashboard.maddy-response-surface-acceptance.v1", version: DASHBOARD_VERSION, buildId: MADDY_RESPONSE_SURFACE_BUILD_ID, passed, total: checks.length, checks };
    console.table(checks);
    console.info(`[MEOS ${DASHBOARD_VERSION}] Maddy Response Surface: ${result.success ? "PASS" : "FAIL"} (${passed}/${checks.length}).`);
    return result;
  }
